            ...
            <div className="border border-[#A7A7A7] rounded-2xl p-6 mt-6">
                ...
                <div className="flex flex-col sm:flex-row sm:justify-between gap-4 sm:items-center">
                    <div className="flex flex-wrap items-center gap-3">
                        <div>
                            <pkg.icon size={16} strokeWidth={0.7} className="w-10 h-10 shrink-0" />
                        </div>
                        <div>
                        ...
                        <span className="bg-[#22C55E1A] text-[#22C55E] text-xs px-4 py-1 rounded-lg whitespace-nowrap">
                            {pkg.status}
                        </span>
                    </div>
                    <button className="bg-[#FF4D4D] w-full sm:w-auto px-6 py-2.5 rounded-md tracking-tight text-xs font-bold hover:bg-black border border-[#FF4D4D] transition">
                        Upgrade Package {data?.upgradeOption}
                    </button>
                </div>
                <div className="mt-6">
                    <p className="text-xl">{pkg.remaining}</p>
                    <div className="flex flex-col xs:flex-row xs:justify-between gap-1 xs:items-center text-base sm:text-lg font-light mb-2">
                        <p>
                            Sessions Remaining Out Of {pkg.total}
                        </p>
                        <span className="text-[#FF4D4D] whitespace-nowrap">
                            {completed} Completed
                        </span>
                    </div>
                ...
                <div className="mt-6">
                    <h3 className="font-semibold text-xl sm:text-2xl mb-3">Package Includes:</h3>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                        {features.map((f, i) => (
                            <p key={i} className="flex items-center text-[#FFFFFF] text-base sm:text-xl font-light">
                                <span className="text-[#FF4D4D] mr-2 shrink-0"><Check size={19} /></span>
                                {f}
                            </p>
                        ))}
                    </div>
